...
func reviewPrompt(slug, version, schema, body string) string {
	...
	b.WriteString("matching this schema exactly (`verdict` is `adopt` if an agent could build from the\n")
	b.WriteString("document today, `revise` if any axis names a blocking gap; `top_gaps` is the ordered\n")
